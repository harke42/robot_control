...
camera_matrix = np.asarray([[559.933357702163, 0.0, 640.5283169916149], [0.0, 556.8863039163979, 362.6814257152788], [0.0, 0.0, 1.0]])
dist_coeff = np.asarray([[-0.05152339363259177, 0.1397406416805854, -0.002481788522966798, 0.0002377054578621766, -0.09210858402238659]])


print("Initializing Camera", file=sys.stderr)
try:
    picam = picamera2.Picamera2()
    picam_config = picam.create_video_configuration(raw={"size": (2304, 1296)}, main={"format": "RGB888", "size": (1280, 720)}, buffer_count=5)
    ...
    if frame is not None:
        try:
            ...
            if marker_ids is not [] and marker_ids is not None:
                rvecs, tvecs, objpts = cv2.aruco.estimatePoseSingleMarkers(marker_corners, MARKER_SIZE, camera_matrix, dist_coeff)
